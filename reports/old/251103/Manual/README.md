...
| [EXECUTION_AGENTS_GUIDE.md](EXECUTION_AGENTS_GUIDE.md) | 18p | 30+ | ✅ Mermaid | Execution Agents 상세 가이드 |
| [STATE_MANAGEMENT_GUIDE.md](STATE_MANAGEMENT_GUIDE.md) | 25p | 40+ | ✅ Mermaid | State 관리 전략 |
| [TOOLS_REFERENCE.md](TOOLS_REFERENCE.md) | 15p | 25+ | ✅ Mermaid | Tools 레퍼런스 |

#### 📊 시스템 분석 보고서 (2025-10-30)

| 문서 | 페이지 | Mermaid | 설명 |
|------|--------|---------|------|
| ⭐ **[SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md)** | 30p | ✅ 5개 | **전체 시스템 플로우 상세 분석**<br/>Frontend → Backend → Database 완전한 흐름 |
| ⭐ **[SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md)** | 28p | ✅ 5개 | **Service Agent 내부 플로우 분석**<br/>TeamSupervisor, Agents, Tools 내부 동작 |
| [CHATBOT_COMPREHENSIVE_SYSTEM_ANALYSIS_251030.md](CHATBOT_COMPREHENSIVE_SYSTEM_ANALYSIS_251030.md) | 35p | ❌ | 전체 시스템 종합 분석 (15개 섹션) |
| [CHATBOT_STRUCTURE_VERIFICATION_251030.md](CHATBOT_STRUCTURE_VERIFICATION_251030.md) | 12p | ❌ | 매뉴얼 vs 실제 구조 검증 보고서 |
| [CHATBOT_COMPLETE_FLOW_MANUAL.md](CHATBOT_COMPLETE_FLOW_MANUAL.md) | 45p | ✅ Text | 챗봇 완전한 흐름 매뉴얼 (원본) |

**총 문서량**: ~231 페이지 (A4 기준), 200+ 코드 예시, 30+ Mermaid 다이어그램
...
```mermaid
flowchart LR
    A[1. ARCHITECTURE_OVERVIEW] --> B[2. SYSTEM_COMPLETE_FLOW_251030]
    B --> C[3. SERVICE_AGENT_FLOW_251030]
    C --> D[4. DATABASE_GUIDE]
    D --> E[5. API_REFERENCE]
...
**⭐ 추천**: 2025-10-30 작성된 상세 플로우 문서를 먼저 읽으면 전체 시스템을 빠르게 이해할 수 있습니다.

#### 📚 심화 학습 (1주일)

```mermaid
flowchart LR
    F[6. EXECUTION_AGENTS_GUIDE] --> G[7. STATE_MANAGEMENT_GUIDE]
    G --> H[8. TOOLS_REFERENCE]
    H --> I[9. DEPLOYMENT_GUIDE]
...
#### **2단계: 전체 시스템 플로우 이해 (2-3시간)** ⭐ NEW
📖 [SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md)
- **Frontend → Backend → Database 완전한 흐름** (3.9초 타임라인)
- WebSocket 연결부터 최종 응답까지 11개 Phase 상세 분석
- 5개 Mermaid 다이어그램으로 시각화
- LangGraph 노드 실행 순서 및 조건부 라우팅
- HITL 흐름 및 에러 처리 전략
- 성능 분석 (병목 지점 2개 파악)

#### **3단계: Service Agent 내부 동작 이해 (2-3시간)** ⭐ NEW
📖 [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md)
- **TeamSupervisor, Agents, Tools 내부 구조** 완전 분석
- 5개 Mermaid 다이어그램으로 시각화
- **4단계 Fallback 전략** (Hardcoded → LLM → Simplified → Defaults)
- SearchExecutor 병렬 검색 메커니즘
- **HybridLegalSearch** (FAISS + SQLite 병합 알고리즘)
- Agent Registry 패턴 및 Tool 통합 패턴
...
### ⭐ NEW: [SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md)

**전체 시스템 플로우 상세 분석 (2025-10-30 작성)**

#### 주요 내용
- ✅ **Frontend → Backend → Database 완전한 흐름**
  - Phase 1: 세션 초기화 (REST API)
  - Phase 2: WebSocket 연결
  - Phase 3: 쿼리 수신 및 처리
  - Phase 4: Supervisor 실행 (LangGraph 5개 노드)
  - Phase 5: 최종 응답
...
- ✅ **타임라인 분석** (3.9초 전체 흐름)
  ...
  - 10ms: WebSocket 전송
  - 25ms: Supervisor 호출
  - 100ms-1500ms: Planning (Intent 분석 + Agent 선택)
  - 1600ms-2100ms: Tool 실행 (FAISS + SQLite)
  - 2300ms-3800ms: 응답 생성 (LLM)
  - 3900ms: 최종 응답 전송

- ✅ **5개 Mermaid 다이어그램**
  1. System Components (전체 아키텍처)
  2. Sequence Diagram (시간순 흐름)
  3. LangGraph Node Execution (노드 실행 경로)
  4. Error Handling Flow (4계층 에러 처리)
  5. HITL Workflow (체크포인트 관리)

- ✅ **성능 분석**
  - 병목 1: LLM 호출 (2800ms, 71.8%)
  - 병목 2: FAISS 검색 (200ms, 5.1%)
  - 개선 방안: 캐싱, 병렬화, 스트리밍, Model 최적화

#### Mermaid 다이어그램
- ✅ 5개의 상세 다이어그램 (괄호 없이 호환성 확보)
...
### ⭐ NEW: [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md)

**Service Agent 내부 플로우 상세 분석 (2025-10-30 작성)**

#### 주요 내용
- ✅ **Agent 아키텍처**
  - Agent Registry 패턴 (중앙 집중식 관리)
  - Agent Adapter 패턴 (표준화된 인터페이스)
  - Tool 통합 패턴 (14개 Tools)

- ✅ **Supervisor 워크플로우**
  - LangGraph 그래프 구성 (5개 노드)
  - 조건부 라우팅 (IRRELEVANT → respond, 나머지 → execute)
  - State 관리 (MainSupervisorState)

- ✅ **Planning Phase 상세**
  - Intent 분석 (15개 IntentType)
  - Long-term Memory 로딩
  - ExecutionStep 생성

- ✅ **4단계 Fallback 전략** (Agent Selection)
  - Stage 0: 하드코딩 키워드 필터 (즉시 매칭)
  - Stage 1: LLM Agent Selection (JSON 응답)
  - Stage 2: Simplified LLM (텍스트 파싱)
  - Stage 3: Safe Defaults (항상 성공)

- ✅ **SearchExecutor 내부 동작**
  - 2단계 키워드 추출 (LLM → 패턴 매칭)
  - 병렬 검색 실행 (asyncio.gather)
  - 에러 복원력 (부분 실패 시 빈 결과로 계속 진행)

- ✅ **HybridLegalSearch 메커니즘**
  - FAISS 벡터 검색 (의미 기반)
  - SQLite 키워드 검색 (정확한 용어 매칭)
  - 결과 병합 알고리즘 (중복 제거 → 점수 정렬 → top_k 선택)

- ✅ **5개 Mermaid 다이어그램**
  1. Service Agent Layer Structure
  2. LangGraph Graph Construction
  3. 4-Stage Fallback Strategy
  4. SearchExecutor Internal Flow
  5. HybridLegalSearch Mechanism
...
| 개념 | 설명 | 문서 |
|------|------|------|
| **TeamBasedSupervisor** | LangGraph 기반 중앙 조정자 (Singleton) | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
| **PlanningAgent** | 의도 분석 및 실행 계획 수립 (15개 IntentType) | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
| **SearchExecutor** | 법률, 시세, 대출 병렬 검색 실행 | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
| **HybridLegalSearch** | FAISS + SQLite 하이브리드 검색 | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
| **4-Stage Fallback** | Agent 선택 폴백 전략 | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
| **LangGraph Workflow** | 5개 노드 + 조건부 라우팅 | [SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md) |
| **HITL Flow** | Human-in-the-Loop 워크플로우 | [SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md) |
| **Checkpointing** | LangGraph State 저장/복구 (PostgreSQL) | [DATABASE_GUIDE.md](DATABASE_GUIDE.md) |
| **pgvector** | PostgreSQL 벡터 검색 확장 | [DATABASE_GUIDE.md](DATABASE_GUIDE.md) |
| **WebSocket Protocol** | 실시간 통신 프로토콜 (7가지 메시지 타입) | [API_REFERENCE.md](API_REFERENCE.md) |
| **Progress Flow** | 실시간 진행 상황 UI | [SYSTEM_COMPLETE_FLOW_251030.md](SYSTEM_COMPLETE_FLOW_251030.md) |
| **Agent Registry** | 중앙 집중식 Agent 관리 패턴 | [SERVICE_AGENT_FLOW_251030.md](SERVICE_AGENT_FLOW_251030.md) |
...
| 버전 | 날짜 | 변경 사항 |
|------|------|----------|
| 2.0 | 2025-10-30 | ⭐ **대규모 업데이트**<br/>- SYSTEM_COMPLETE_FLOW_251030.md 추가 (30p, 5개 Mermaid)<br/>- SERVICE_AGENT_FLOW_251030.md 추가 (28p, 5개 Mermaid)<br/>- CHATBOT_COMPREHENSIVE_SYSTEM_ANALYSIS_251030.md 추가 (35p)<br/>- CHATBOT_STRUCTURE_VERIFICATION_251030.md 추가 (12p)<br/>- 총 105페이지 추가, 10개 Mermaid 다이어그램 추가 |
...
```bash
# 1. 문서 읽기 (⭐ 추천: 2025-10-30 신규 문서부터 시작)
cat SYSTEM_COMPLETE_FLOW_251030.md        # 전체 시스템 플로우 (30p)
cat SERVICE_AGENT_FLOW_251030.md          # Service Agent 내부 동작 (28p)
cat ARCHITECTURE_OVERVIEW.md              # 아키텍처 개요
